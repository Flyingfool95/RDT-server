...
import { sendResponse, getSecureBody } from "../../utils/helpers.ts";
...
export async function resetPassword(ctx: Context): Promise<void> {
    const body = (await getSecureBody(ctx, loginSchema)) as { token: string; password: string };

    const verifiedRefreshToken = (await verifyJWT(body.token)) as { email: string };
