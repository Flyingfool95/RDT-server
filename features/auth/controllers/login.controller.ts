...
import { getSecureBody, getUserIfExists, sendResponse } from "../../utils/helpers.ts";
...
export async function login(ctx: Context): Promise<void> {
    const body = (await getSecureBody(ctx, loginSchema)) as { email: string; password: string };
    ...
    const isMatch = await verify(userData.password as string, body.password);
    if (!isMatch) {
